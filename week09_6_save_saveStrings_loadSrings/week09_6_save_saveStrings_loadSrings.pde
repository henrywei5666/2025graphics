//week09_6_save_saveStrings_loadSrings
void setup(){
  size(300,300);
  rect(10,10,80,80);
  rect(110,110,80,80);
...
ArrayList<String> lines = new ArrayList<String>(); //Array資料結構
String [] another; //另外一個獨到的字串的資料結構
void keyPressed(){
  String [] arr = new String[lines.size()]; //傳統Java陣列的資料結構
  lines.toArray(arr); //把ArrayList轉換成傳統的陣列,以便存檔
  if(key=='s') saveStrings("lines.txt",arr); //按下英文小寫s
}
